...
class CubeDataProcessor:
    ...
    def create_sidebar(self):
        ...
        title_label = ctk.CTkLabel(self.sidebar, text="CUBE DATA\nPROCESSOR", 
                                   font=ctk.CTkFont(size=22, weight="bold"))
        title_label.grid(row=1, column=0, padx=20, pady=(0, 30))
        
        # Mode selection
        mode_label = ctk.CTkLabel(self.sidebar, text="⚙️ Processing Mode", 
                                  font=ctk.CTkFont(size=15, weight="bold"), anchor="w")
        mode_label.grid(row=2, column=0, padx=20, pady=(10, 15), sticky="ew")
        ...
        self.grade_radio.grid(row=3, column=0, padx=30, pady=8, sticky="w")
        
        self.date_radio = ctk.CTkRadioButton(self.sidebar, text="Date Only", 
                                            variable=self.mode_var, value="date_only",
                                            command=self.update_mode_ui,
                                            font=ctk.CTkFont(size=13))
        self.date_radio.grid(row=4, column=0, padx=30, pady=8, sticky="w")
        
        self.both_radio = ctk.CTkRadioButton(self.sidebar, text="Grade + Date", 
                                            variable=self.mode_var, value="both",
                                            command=self.update_mode_ui,
                                            font=ctk.CTkFont(size=13))
        self.both_radio.grid(row=5, column=0, padx=30, pady=8, sticky="w")
        
        # Grade files
        grade_label = ctk.CTkLabel(self.sidebar, text="📁 Grade Files", 
                                   font=ctk.CTkFont(size=15, weight="bold"), anchor="w")
        grade_label.grid(row=6, column=0, padx=20, pady=(25, 10), sticky="ew")
        
        self.grade_listbox = ctk.CTkTextbox(self.sidebar, height=120, font=ctk.CTkFont(size=11))
        self.grade_listbox.grid(row=7, column=0, padx=20, pady=(0, 10), sticky="ew")
        
        self.update_grade_listbox()
        
        grade_btn_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        grade_btn_frame.grid(row=8, column=0, padx=20, pady=(0, 20), sticky="ew")
        
        add_grade_btn = ctk.CTkButton(grade_btn_frame, text="➕ Add", 
                                      command=self.add_grades, width=100,
                                      font=ctk.CTkFont(size=12, weight="bold"))
        add_grade_btn.pack(side="left", padx=(0, 8))
        ...
        social_label.grid(row=11, column=0, padx=20, pady=(20, 10), sticky="ew")
        ...
        github_btn.grid(row=12, column=0, padx=20, pady=5)
        ...
        insta_btn.grid(row=13, column=0, padx=20, pady=(5, 30))
    ...
    def create_main_content(self):
        ...
        self.calendar_frame.grid(row=0, column=0, sticky="ew", padx=15, pady=12)
        ...
        calendar_label.grid(row=0, column=0, padx=15, pady=15, sticky="w")
        
        self.calendar_entry = ctk.CTkEntry(self.calendar_frame, textvariable=self.calendar_path, 
                                           placeholder_text="Select calendar Excel file...",
                                           height=40, font=ctk.CTkFont(size=12))
        self.calendar_entry.grid(row=0, column=1, padx=10, pady=15, sticky="ew")
        
        calendar_btn = ctk.CTkButton(self.calendar_frame, text="Browse", 
                                     command=self.pick_calendar, width=110, height=40,
                                     font=ctk.CTkFont(size=12, weight="bold"))
        calendar_btn.grid(row=0, column=2, padx=15, pady=15)
        
        # Office file
        office_frame = ctk.CTkFrame(self.main_frame)
        office_frame.grid(row=1, column=0, sticky="ew", padx=15, pady=12)
        office_frame.grid_columnconfigure(1, weight=1)
        
        office_label = ctk.CTkLabel(office_frame, text="📄 Office Format File", 
                                    font=ctk.CTkFont(size=15, weight="bold"))
        office_label.grid(row=0, column=0, padx=15, pady=15, sticky="w")
        
        self.office_entry = ctk.CTkEntry(office_frame, textvariable=self.office_path, 
                                         placeholder_text="Select office template file...",
                                         height=40, font=ctk.CTkFont(size=12))
        self.office_entry.grid(row=0, column=1, padx=10, pady=15, sticky="ew")
        
        office_btn = ctk.CTkButton(office_frame, text="Browse", 
                                   command=self.pick_office, width=110, height=40,
                                   font=ctk.CTkFont(size=12, weight="bold"))
        office_btn.grid(row=0, column=2, padx=15, pady=15)
        
        # Output folder
        output_frame = ctk.CTkFrame(self.main_frame)
        output_frame.grid(row=2, column=0, sticky="ew", padx=15, pady=12)
        ...
        self.output_entry = ctk.CTkEntry(output_frame, textvariable=self.output_path, 
                                         placeholder_text="Select output destination...",
                                         height=40, font=ctk.CTkFont(size=12))
        self.output_entry.grid(row=0, column=1, padx=10, pady=15, sticky="ew")
        
        output_btn = ctk.CTkButton(output_frame, text="Browse", 
                                   command=self.pick_output_folder, width=110, height=40,
                                   font=ctk.CTkFont(size=12, weight="bold"))
        output_btn.grid(row=0, column=2, padx=15, pady=15)
        
        # Start button
        self.start_btn = ctk.CTkButton(self.main_frame, text="▶️  START PROCESSING", 
                                       command=self.run_processing,
                                       font=ctk.CTkFont(size=20, weight="bold"),
                                       height=60, fg_color="#16a34a", hover_color="#15803d")
        self.start_btn.grid(row=3, column=0, padx=15, pady=25, sticky="ew")
        
        # Progress bar
        self.progress = ctk.CTkProgressBar(self.main_frame, height=12)
        self.progress.grid(row=4, column=0, sticky="ew", padx=15, pady=(0, 15))
        self.progress.set(0)
        
        # Log section
        log_label = ctk.CTkLabel(self.main_frame, text="📋 Processing Log", 
                                font=ctk.CTkFont(size=15, weight="bold"), anchor="w")
        log_label.grid(row=5, column=0, padx=15, pady=(10, 5), sticky="w")
        
        self.log_textbox = ctk.CTkTextbox(self.main_frame, font=ctk.CTkFont(size=11))
        self.log_textbox.grid(row=6, column=0, sticky="nsew", padx=15, pady=(0, 15))
        
        # Footer
        footer_label = ctk.CTkLabel(self.main_frame, 
                                    text="© 2026 Sandeep | github.com/Sandeep2062/Cube-Data-Processor", 
                                    font=ctk.CTkFont(size=11), text_color="gray60")
        footer_label.grid(row=7, column=0, pady=(5, 10))
    ...
    def update_grade_listbox(self):
        ...
        if not self.grade_files:
            self.grade_listbox.insert("end", "No files selected")
        else:
